...
func assertQuery(t *testing.T, query string, tokens []Token) {
	...
	for i := 0; ; i++ {
		...
		if token.Type != tokens[i].Type {
			t.Fatalf("Failed parse query (%s). expected %v got %v: %v", query, tokens[i].Type, token.Type, tokens)
		}
		...
		cases := []struct {
			...
			},
			{
				...
				[]Token{
					...
					{Type: IDENT, Value: "int", Position: Position{Line: 1, Offset: 23, Column: 3}},
		...
		cases := []struct {
		...
		}{
			{
				...
				[]Token{
					...
					{Type: IDENT, Value: "varchar", Position: Position{Line: 1, Offset: 34, Column: 7}},
